#!/usr/bin/env python3
"""Generate agents/shared/generated/verbs_blob.cpp from protocol spec JSON.
...
The generated file is compiled into the RH_MCP builds (windows-modern and,
as of Phase 2.0, windows-legacy) from a shared path. windows-classic does not
compile it (it has no MCP framing); the verb set is still filtered by
x-families.windows-modern.implemented — see is_modern_implemented().
...
# Phase 2.0: the generated blob is shared. windows-modern and windows-legacy
# both compile it (RH_MCP); a single shared path keeps one generator output
# and one custom-command dependency instead of duplicating per family.
OUT_FILE  = REPO_ROOT / "agents" / "shared" / "generated" / "verbs_blob.cpp"
